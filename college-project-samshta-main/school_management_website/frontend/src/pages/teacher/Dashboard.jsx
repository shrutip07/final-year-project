...
//   // Load all academic years (from students in assignments)
//   useEffect(() => {
//     const token = localStorage.getItem("token");
//     if (!token) return;
//     axios
...
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       alert("Marked this academic year as completed. Clerk can now reassign you.");
//     } catch {
...
export default function TeacherDashboard() {
  ...
  const [error, setError] = useState("");
 const [classes, setClasses] = useState([]); // [{ academic_year, standard, division }]
 ...
 // Load classes (standard/division) for selected year
useEffect(() => {
if (!academicYear) return;
const token = localStorage.getItem("token");
if (!token) {
...
axios.get(`http://localhost:5000/api/teacher/classes?academic_year=${encodeURIComponent(academicYear)}`,
{ headers: { Authorization: `Bearer ${token}` } }
)
.then(res => {
setClasses(res.data || []);
          ...
          <div>
            ...
            <div className="card">
              <div
                ...
                style={{ gap: "12px", flexWrap: "wrap" }}
              >
                <h3 style={{ flex: 1 }}>{t("my_classes")}</h3>
                <select
                  value={academicYear}
                  onChange={e => setAcademicYear(e.target.value)}
                  className="form-control"
